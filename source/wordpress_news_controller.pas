...
procedure TWPNewsWebModule.RequestHandler(Sender: TObject; ARequest: TRequest;
...
begin
  ...
  ;
  category_permalink := StringReplace(Parameter.Values['category'],
    '"', '', [rfReplaceAll]);
  ;
  ...
  begin
    ...
    begin
      AddJoin(AppData.tablePrefix + '_term_taxonomy', 'term_id',
        AppData.tablePrefix + '_terms.term_id', ['taxonomy']);
  ...
  begin
    _News.AddJoin(AppData.tablePrefix + '_term_relationships', 'object_id',
      AppData.tablePrefix + '_posts.ID', ['object_id']);
    _News.Find(['post_type="post"', 'post_status="publish"',
      AppData.tablePrefix + '_term_relationships.term_taxonomy_id=' + i2s(category_id)],
  ...
  begin
    ...
    begin
      url := '/' + FormatDateTime('YYYY', _News['post_date']) +
        '/' + FormatDateTime('mm', _News['post_date']) + '/' +
        _News['post_name'] + '/';
      lst.Add('<li' + item_class + '><a href="' + url + '">' +
        _News['post_title'] + '</a>');
...
begin
  ...
  begin
    ...
    begin
      AddJoin(AppData.tablePrefix + 'term_relationships', 'object_id',
        AppData.tablePrefix + 'posts.ID', ['term_taxonomy_id']);
      AddJoin(AppData.tablePrefix + 'terms', 'term_id',
        AppData.tablePrefix + 'term_relationships.term_taxonomy_id', ['slug']);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          html := html + #13#10'  <li><a href="' + BaseURL + '" class="on">' +
            __('Home') + '</a></li>';
        end
        else
        begin
      ...
      begin
        ...
        title := '';
        url := '#';
        submenu := GeneratePagesMenu(Value['ID'], Parameter,
          ParentBaseURL + Value['post_name'] + '/', False);
        if submenu = '' then
        ...
        begin
          if IsParent then
          begin
            html := html + #13#10'<li id="menu-item-' + string(
              Value['ID']) + '" class="menu-item ' + page_item_has_children + '">';
          end
          else
          begin
            html := html + #13#10'  <li id="menu-item-' + string(
...
begin
  ...
  News.AddJoin(AppData.tablePrefix + 'users', 'ID',
    AppData.tablePrefix + 'posts.post_author', ['display_name']);
  News.Find(['date_format( post_date, "%Y%m") = "' + _GET['year'] +
    _GET['month'] + '"', 'post_status="publish"', 'post_type="post"',
    'post_name = "' + _GET['permalink'] + '"'],
  ...
  begin
    thumbnail_url := Config.GetValue('wordpress/base_url', '') +
      Config.GetValue('wordpress/path', '') + '/' + thumbnail_url;
